...
std::shared_ptr<physics2::Obstacle> CharacterController::makePart(int part, int connect, const Vec2 &pos) {
    ...
    if (part == PART_BODY) {
        ...
        float oct[] = {-w, -h * 0.414f, -w * 0.414f, -h, w * 0.414f, -h, w, -h * 0.414f, w, h * 0.414f, w * 0.414f, h, -w * 0.414f, h, -w, h * 0.414f};
...
void CharacterController::setDrawScale(float scale) { _drawScale = scale; }
...
void CharacterController::enableMotor() { _motorEnabled = true; }

void CharacterController::disableMotor() { _motorEnabled = false; }
...
#pragma mark camera
    ...
    for (int i = 0; i < _obstacles.size(); i++) {
        ...
        std::shared_ptr<cugl::Texture> bodySprite;

        // TODO: Put this in macros
        if (i == PART_BODY) {
            bodySprite = getTextureForPart(i, _color); // TODO: Make return a texture
            if (_color == "black") {
                _bodyNode = scene2::SpriteNode::allocWithSheet(bodySprite, 9, 9, 75);
            } else {
                _bodyNode = scene2::SpriteNode::allocWithSheet(bodySprite, 5, 5, 24);
            }
...
#pragma mark ChangeColor Action
    ...
    void CharacterController::drawCharacterLines(float scale) {
        if (_obstacles.size() >= PART_LH + 1) {
            ...
            Vec2 leftshoulder = _obstacles[PART_BODY]->getPosition() +
                                Vec2(leftShoulderOffset.x * cos(body_angle) - leftShoulderOffset.y * sin(body_angle), leftShoulderOffset.x * sin(body_angle) + leftShoulderOffset.y * cos(body_angle));
            Vec2 rightshoulder = _obstacles[PART_BODY]->getPosition() + Vec2(rightShoulderOffset.x * cos(body_angle) - rightShoulderOffset.y * sin(body_angle),
                                                                             rightShoulderOffset.x * sin(body_angle) + rightShoulderOffset.y * cos(body_angle));
    ...
    std::string CharacterController::getColor() { return this->_color; }

    // TODO: remove all alloc() here
    std::shared_ptr<cugl::Texture> CharacterController::getTextureForPart(int partId, const std::string &color) {
        ...
        if (partId == PART_BODY) {
            if (color == "black") {
                ...
                textureNode = _blacktextures;
            } else if (color == "orange") {
                textureNode = _orangetextures;
            } else if (color == "pink") {
                textureNode = _pinktextures;
            } else if (color == "purple") {
                textureNode = _purpletextures;
            } else if (color == "green") {
                textureNode = _greentextures;
            } else if (color == "blue") {
                textureNode = _bluetextures;
            }
        }
