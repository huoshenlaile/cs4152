...
void CharacterController::drawCharacterLines(float scale) {
    if (_obstacles.size() >= PART_LH + 1) {
        ...
        Vec2 rightshoulder = _obstacles[PART_BODY]->getPosition() +
                             Vec2(rightShoulderOffset.x * cos(body_angle) - rightShoulderOffset.y * sin(body_angle), rightShoulderOffset.x * sin(body_angle) + rightShoulderOffset.y * cos(body_angle));
