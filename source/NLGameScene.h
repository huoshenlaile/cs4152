 ...
 *
 * This class is used to support automatically syncing newly added obstacle
 * mid-simulation. Obstacles added throught the ObstacleFactory class from one
 * client will be added to all clients in the simulations.
 */
...
public:
   ...
   */
  static std::shared_ptr<CrateFactory>
  alloc(std::shared_ptr<AssetManager> &assets) {
  ...
  createObstacle(Vec2 pos, float scale);

  /**
   * Helper method for converting normal parameters into byte vectors used for
   * syncing.
   */
  std::shared_ptr<std::vector<std::byte>> serializeParams(Vec2 pos,
                                                          float scale);
...
#pragma mark Internal Object Management
   ...
   *
   * This method adds a listener so that the sceneNode will move along with the
   * obstacle.
   */
  ...
  /**
   * This method adds a crate that had been fired by the player's cannon amid
   * the simulation.
   *
