 ...
 */
std::shared_ptr<NetEvent> CharacterEvent::newEvent(){
    return std::make_shared<CrateEvent>();
}

std::shared_ptr<NetEvent> CharacterEvent::allocCharacterEvent(Vec2 pos){
    //TODO: make a new shared copy of the event and set its _pos to pos.
#pragma mark BEGIN SOLUTION
    auto event = std::make_shared<CharacterEvent>();
    event->_pos = pos;
    return event;
 ...
 */
std::vector<std::byte> CharacterEvent::serialize(){
    //TODO: serialize _pos
#pragma mark BEGIN SOLUTION
    _serializer.reset();
 ...
 */
void CharacterEvent::deserialize(const std::vector<std::byte>& data){
