...
/** The wall vertices */
float WALL1[] = {0.0f,  0.0f,  16.0f, 0.0f,  16.0f, 1.0f,  3.0f, 1.0f,
                 3.0f,  5.0f,  2.0f,  7.0f,  1.0f,  17.0f, 8.0f, 15.0f,
                 16.0f, 17.0f, 16.0f, 18.0f, 0.0f,  18.0f};
float WALL2[] = {32.0f, 18.0f, 16.0f, 18.0f, 16.0f, 17.0f, 31.0f, 16.0f, 30.0f,
                 10.0f, 31.0f, 1.0f,  16.0f, 1.0f,  16.0f, 0.0f,  32.0f, 0.0f};
float WALL3[] = {4.0f, 9.5f, 8.0f, 9.5f, 8.0f, 10.5f, 4.0f, 10.5f};

/** The positions of the crate pyramid */
float BOXES[] = {14.5f, 14.25f, 13.0f, 12.00f, 16.0f, 12.00f, 11.5f, 9.75f,
                 14.5f, 9.75f,  17.5f, 9.75f,  13.0f, 7.50f,  16.0f, 7.50f,
                 11.5f, 5.25f,  14.5f, 5.25f,  17.5f, 5.25f,  10.0f, 3.00f,
                 13.0f, 3.00f,  16.0f, 3.00f,  19.0f, 3.0f};
...
float WALL[WALL_COUNT][WALL_VERTS] = {{16.0f, 18.0f, 0.0f, 18.0f, 0.0f, 0.0f,
 ...
 */
std::pair<std::shared_ptr<physics2::Obstacle>,
          std::shared_ptr<scene2::SceneNode>>
CrateFactory::createObstacle(Vec2 pos, float scale) {
...
/**
 * Helper method for converting normal parameters into byte vectors used for
 * syncing.
 */
std::shared_ptr<std::vector<std::byte>>
CrateFactory::serializeParams(Vec2 pos, float scale) {
  // TODO: Use _serializer to serialize pos and scale (remember to make a shared
  // copy of the serializer reference, otherwise it will be lost if the
  // serializer is reset).
#pragma mark BEGIN SOLUTION
  _serializer.reset();
...
std::pair<std::shared_ptr<physics2::Obstacle>,
          std::shared_ptr<scene2::SceneNode>>
CrateFactory::createObstacle(const std::vector<std::byte> &params) {
  // TODO: Use _deserializer to deserialize byte vectors packed by {@link
  // serializeParams()} and call the regular createObstacle() method with them.
#pragma mark BEGIN SOLUTION
  _deserializer.reset();
...
bool GameScene::init(
    ...
    const std::shared_ptr<cugl::physics2::net::NetEventController> network,
    bool isHost) {
...
bool GameScene::init(const std::shared_ptr<AssetManager> &assets,
  ...
  _chargeBar = std::dynamic_pointer_cast<scene2::ProgressBar>(
  ...
  _world = physics2::net::NetWorld::alloc(
      Rect(0, 0, DEFAULT_WIDTH, DEFAULT_HEIGHT), Vec2(0, DEFAULT_GRAVITY));
  ...
  // Make a std::function reference of the linkSceneToObs function in game scene
  // for network controller
  std::function<void(const std::shared_ptr<physics2::Obstacle> &,
                     const std::shared_ptr<scene2::SceneNode> &)>
      linkSceneToObsFunc = [=](const std::shared_ptr<physics2::Obstacle> &obs,
                               const std::shared_ptr<scene2::SceneNode> &node) {
        this->linkSceneToObs(obs, node);
      };

  /**
   * TODO: Call network controller to enable physics with linkSceneToObsFunc as
   * the method to attach nodes to obstacles and attach the crate factory to the
   * physics controller and set _factId to its return value.
   *
...
/**
 * This method adds a crate that had been fired by the player's cannon amid the
 * simulation.
 *
 * If this machine is host, the crate should be fire from the left cannon
 * (_cannon1), vice versa.
 */
void GameScene::fireCrate() {
  // TODO: Add a new crate to the simulation using the addSharedObstacle()
  // method from the physics controller, and set its velocity in the direction
  // the cannon is aimed scaled by (50 * _input.getFirePower()). HINT: You can
  // use the serializedParams() method of the crate factory to help you
...
/**
 * This method takes a characterEvent and processes it.
 */
void GameScene::processCharacterEvent(
    const std::shared_ptr<CharacterEvent> &event) {
  ...
  // want to use addSharedObstacle() for it because it will create two separate
  // crate. Instead you should use addInitObstacle(), which has the same top-bit
  // id and if all clients called init obstacle the same amount of times, the
  // same low-bit id. There is a potential race condition where multiple clients
  // calling addInitObstacle() can cause id to be mixed up(clients send
  // CrateEvent at the same time). In this lab, we will not address that race
  // condition. But you could send along an obstacle id to ensure that all
  // clients have that id for the obstacle.
}
...
void GameScene::processCrateEvent(const std::shared_ptr<CrateEvent> &event) {
  ...
  auto crate = physics2::BoxObstacle::alloc(
      Vec2(event->getPos().x, event->getPos().y), boxSize);
...
#pragma mark : Ragdoll
  // Allocate the ragdoll and set its (empty) node. Its model handles creation
  // of parts (both obstacles and nodes to be drawn) upon alllocation and
  // setting the scene node.
...
#pragma mark : Wall polygon 1
  ...
  wallobj1 =
      physics2::PolygonObstacle::allocWithAnchor(wall1, Vec2::ANCHOR_CENTER);
...
#pragma mark : Wall polygon 2
  ...
  wallobj2 =
      physics2::PolygonObstacle::allocWithAnchor(wall2, Vec2::ANCHOR_CENTER);
...
#pragma mark : Cannon
  ...
  //    addInitObstacle(wallobj1, wallsprite1);  // All walls share the same
  //    texture addInitObstacle(wallobj2, wallsprite2);  // All walls share the
  //    same texture addInitObstacle(_cannon1, _cannon1Node);
...
#pragma mark : Walls
  ...
  for (int ii = 0; ii < WALL_COUNT; ii++) {
    ...
    wallobj =
        physics2::PolygonObstacle::allocWithAnchor(wall, Vec2::ANCHOR_CENTER);
...
#pragma mark : Platforms
  for (int ii = 0; ii < PLATFORM_COUNT; ii++) {
    ...
    platobj = physics2::PolygonObstacle::allocWithAnchor(platform,
                                                         Vec2::ANCHOR_CENTER);
    // You cannot add constant "".  Must stringify
...
void GameScene::addObstacle(
    const std::shared_ptr<cugl::physics2::Obstacle> &obj,
    const std::shared_ptr<cugl::scene2::SceneNode> &node, bool useObjPosition) {
  ...
  if (obj->getBodyType() == b2_dynamicBody) {
    scene2::SceneNode *weak =
        node.get(); // No need for smart pointer in callback
...
/**
 * Adds the physics object to the physics world and loosely couples it to the
 * scene graph
 *
 ...
 */
void GameScene::addInitObstacle(
    const std::shared_ptr<physics2::Obstacle> &obj,
    const std::shared_ptr<scene2::SceneNode> &node) {
...
void GameScene::preUpdate(float dt) {
  ...
  std::shared_ptr<cugl::physics2::Obstacle> leftArm =
      _ragdoll->getPartObstacle(PART_LEFT_HAND);
  std::shared_ptr<cugl::physics2::Obstacle> rightArm =
      _ragdoll->getPartObstacle(PART_RIGHT_HAND);
  ...
  for (const auto &[key, value] : inputs) {
    if (_input_to_arm.count(key) == 0) { // Add touchpoint
      ...
      if (_arm_to_input.count(PART_LEFT_HAND) == 0 &&
          (leftArm->getPosition() * _scale).distance(pos2d) <
              50 * knob_radius_multiplier) {
        _input_to_arm[key] = PART_LEFT_HAND;
        _arm_to_input[PART_LEFT_HAND] = key;

      } else if (_arm_to_input.count(PART_RIGHT_HAND) == 0 &&
                 (rightArm->getPosition() * _scale).distance(pos2d) <
                     50 * knob_radius_multiplier) {
      ...
      std::shared_ptr<cugl::physics2::Obstacle> arm =
          _ragdoll->getPartObstacle(_input_to_arm[key]);
      arm->setPosition(pos_now / _scale);
      CULog("ARM MOVE COMING");
      _network->pushOutEvent(CharacterEvent::allocCharacterEvent(
          Vec2(DEFAULT_WIDTH / 2, DEFAULT_HEIGHT / 2)));
    }
  }
...
// TODO: if _input.didBigCrate(), allocate a crate event for the center of the
...
void GameScene::fixedUpdate() {
  ...
  // call processCrateEvent if it is a CrateEvent.

  // Hint: You can check if ptr points to an object of class A using
  // std::dynamic_pointer_cast<A>(ptr). You should always check isInAvailable()
  // before popInEvent().
...
#pragma mark BEGIN SOLUTION
  if (_network->isInAvailable()) {
    ...
    } else if (auto charEvent = std::dynamic_pointer_cast<CharacterEvent>(e)) {
      CULog("CHARACTER MOVE GOT");
      processCharacterEvent(charEvent);
    }
  }
#pragma mark END SOLUTION
  _world->update(FIXED_TIMESTEP_S);
 ...
 *
 * Since this game has no real need to determine collisions, right now this is
 * left empty.
 *
 ...
 *
 * This method is called just before Box2D resolves a collision.  We use this
 * method to implement sound on contact, using the algorithms outlined in Ian
 * Parberry's "Introduction to Game Physics with Box2D".
 *
 * @param  contact      The two bodies that collided
...
void GameScene::beforeSolve(b2Contact *contact, const b2Manifold *oldManifold) {
  ...
  if (speed > SOUND_THRESHOLD) {
    ...
    physics2::Obstacle *data1 =
        reinterpret_cast<physics2::Obstacle *>(body1->GetUserData().pointer);
    physics2::Obstacle *data2 =
        reinterpret_cast<physics2::Obstacle *>(body2->GetUserData().pointer);
