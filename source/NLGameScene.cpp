...
CrateFactory::createObstacle(Vec2 pos, float scale) {
  ...
  // TODO: allocate a box obstacle at pos with boxSize, set its angleSnap to 0,
  // debugColor to DYNAMIC_COLOR, density to CRATE_DENSITY, friction to
  // CRATE_FRICTION, and restitution to BASIC_RESTITUTION, after everything is
  // set, make the object shared by calling setShared(). Then allocate a
  // PolygonNode from image, set its anchor to center, and scale to 0.5f. Lastly
  // return the pair of Obstacle and sceneNode.

  // NOTE: When an Obstacle is shared, function calls that change its state are
  // monitored and automatically synchronized. However, every client calling
  // this method is going to run the code above setting the properties. We don't
  // want to share them redundantly, so sharing is turned on afterwards.
...
#pragma mark BEGIN SOLUTION
  if (_input.didBigCrate()) {
    ...
    _network->pushOutEvent(CrateEvent::allocCrateEvent(
