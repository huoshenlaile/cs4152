...
public:
    ...
    static std::shared_ptr<PaintModel> alloc(const std::vector<cugl::Poly2>& paintFrames, const std::vector<Vec2>& locations, const std::vector<std::string>& textures){
        std::shared_ptr<PaintModel> result = std::make_shared<PaintModel>();
        return (result->init(paintFrames, locations, textures) ? result : nullptr);
    }
    
    bool init(const std::vector<cugl::Poly2>& paintFrames, const std::vector<Vec2>& locations, const std::vector<std::string>& textures);
    bool setPolysInScene(const std::shared_ptr<AssetManager>& assets, const std::shared_ptr<cugl::scene2::SceneNode>& worldnode, float scale);
